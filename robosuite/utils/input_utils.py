...
def input2action(device, robot, active_arm="right", env_configuration=None):
    ...
    if controller.use_delta: # Separate processing for delta control
        ...
        dpos, rotation, raw_drotation, grasp = (
            state["dpos"],
            state["rotation"],
            state["raw_drotation"],
            state["grasp"],
    ...
    else: # processing for absolute OSC control
        # TODO: does this need to be different for IK based controllers?

        target_pose = state['target_pose'] # absolute position (x,y,z) + quaternion (x,y,z,w)
        action_pos = target_pose[:3]
        action_axis_angle = T.quat2axisangle(np.array(target_pose[3:]))
        grasp = state['grasp']

        action = np.concatenate([action_pos, list(action_axis_angle), [grasp]])


    # Return the action and grasp
    return action, grasp

def input2teleop_info(device, robot, active_arm="right", env_configuration=None):
    """
    The same as input2action() but with more information
    Args:
        device:
        robot:
        active_arm:
    ...
    teleop_info = dict(save_demo=state["save_demo"], delete_demo=state["delete_demo"], reset=state["reset"])

    # If we're resetting, immediately return this information
    if reset:
        return teleop_info
        # return None, None
